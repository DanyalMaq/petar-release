    ...
    def prepare_inputs_for_multimodal(
        ...
        if vision_tower is None or pets is None or input_ids.shape[1] == 1:
            ...
            image_features = self.encode_images(
                pets, masks, cts, pet_focals, mask_focals, ct_focals
            )

            # Put input_ids on the token embedding device first
            embed_device = self.get_model().embed_tokens.weight.device
            input_ids = input_ids.to(embed_device)

            inputs_embeds = self.get_model().embed_tokens(input_ids)

            # Move image features to the same device as text embeddings
